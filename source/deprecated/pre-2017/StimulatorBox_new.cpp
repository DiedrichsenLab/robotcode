...
// Init: 
// -----------------------------------------------------------
void StimulatorBox::setScale(double s[NUMFINGERS]){ 
	int i;
...
bool StimulatorBox::init(int kind, string filename)
{
	...
	//----set the vots2foce factors correct
	...
	if(inputFile ==0){
		...
		while (!(inputFile.bad() || inputFile.eof())) { 
			inputFile>>scale[i];
			//cout <<scale[kind][i] << " "<< i<<endl;
			i=i+1; 		
		} 

	}
	cout<<endl;
	cout<< "set V2F Box "<<kind<<"==> T " << scale[0] <<"  I "<< scale[1] <<"  M "<< scale[2] <<"  R "<< scale[3] <<"  L "<< scale[4] <<endl; 
	cout<<endl;
	return true; 
} 
