...
double medianETarray[50];			///< 12 blocks per subject, preallocate array to keep track of ETs within session
double ERarray[50];					///< 12 blocks per subject, preallocate array to keep track of ERs within session
...
void MyTrial::read(istream& in) {
	// read from .tgt file
	in >> subNum >> hand >> isTrain >> isChanged >> symbol >> cue >> isMasked >>
		digitChangePos >> digitChangeValue >> execTime >> iti >> precueTime >> windowSize;
...
void MyTrial::writeDat(ostream& out) {
	// write to .dat file
	out << subNum << "\t" << hand << "\t" << isTrain << "\t" << isChanged << 
		"\t" << symbol << "\t" << cue << "\t" << isMasked << "\t" <<
		digitChangePos << "\t" << digitChangeValue << "\t" << precueTime << "\t" << windowSize << "\t";
	int i;
...
void MyTrial::writeHeader(ostream& out) {
	...
	out << "SubNum" << "\t" << "hand" << "\t" << "isTrain" << "\t" << "isChanged" <<
		"\t" << "symbol" << "\t" << "cue" << "\t" <<
		"isMasked" << "\t" << "digitChangePos" << "\t" << "digitChangeValue" <<
