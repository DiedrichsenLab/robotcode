///////////////////////////////////////////////////////////////
/// Single Finger Gain Generalization V2 - Amin Nazerzadeh Ali Ghavampour August 2026
/// Per-trial visual feedback gain (washout/adaptation/test blocks); no cursor perturbation.
///////////////////////////////////////////////////////////////
#include "single_gain_generalization_v2.h" 
	...
	LPSTR kposzArgs, int nWinMode)
{
	...
	gThisInst = hThisInst;
	gExp = new MyExperiment("single_gain_generalization_v2", "single_gain_generalization_v2", "C:/data/EFC_learningDynamics/CAD_Spat_Vis/Single_Finger_Gain_Generalization_V2/"); 
...
double MyTrial::effectiveGain() const {
	return trialGain * forceGain;
}
...
void MyTrial::writeDat(ostream& out) {
	...
	out << subNum << "\t"
		...
		<< feedbackTime << "\t" 
		<< iti << "\t" 
		<< forceGain << "\t"					// Optional global calibration multiplier
...
void MyTrial::updateTextDisplay() {
	...
	// force gains
	sprintf(buffer, "TrialGain = %1.1f  GlobalGain = %1.1f", trialGain, forceGain);
	...
	if (gs.showForces) {
		for (i = 0; i < 5; i++) {
			...
			if (targetForce != 0) {
				...
				forceCursor[i].position[1] = VERT_SHIFT + effectiveGain() * diffForce[i];
...
void MyTrial::control() {
	...
	case WAIT_PLAN: //2
		...
		for (i = 0; i < 5; i++){
			...
			fingerForceTmp = VERT_SHIFT + effectiveGain() * (gBox[0].getForce(i) - gBox[1].getForce(i));
	...
	case WAIT_EXEC:
		...
		if ((beepInterval * 3 + execSampleTime - SAMPLING_DURATION/2) <= gTimer[1] && gTimer[1] < (beepInterval * 3 + execSampleTime + SAMPLING_DURATION/2)) { // NEEDS FIXING
			for (i = 0; i < 5; i++) {
				forceTemps[i] += VERT_SHIFT + effectiveGain() * (gBox[0].getForce(i) - gBox[1].getForce(i));
		...
		if (gTimer[1] >= (beepInterval * 3 + execSampleTime + SAMPLING_DURATION / 2)) { // NEEDS FIXING
			...
			for (i = 0; i < 5; i++) {
				endForces[i] = forceTemps[i] / samplingCounter; // gain-scaled visual cursor position
