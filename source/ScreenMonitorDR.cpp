/******************************************************************
** Screen HD 
** Class for Viewport and Windows Management for Phantom (HD) setup
*******************************************************************/
...
void ScreenMonitor::init(HINSTANCE hInst,HWND hWndParent,int xPos, int yPos, int width,int height,void (*displayFcn)(int))
{
	...
	// create Thread 
	parentHnd = hWndParent;
	hThread = (HANDLE)_beginthreadex( NULL, 0, ScreenMonitorThreadFunc,
				(void *)this, 0, &lThreadId );
	//cout << "Priority: " << GetThreadPriority(hThread)<< ":" <<GetPriorityClass(hThread)<<endl;
...
// ------------------------------------------------------------------------------
// Message loop for ScreenMonitor 
// ------------------------------------------------------------------------------
int ScreenMonitor::MessageLoop()
{
MSG message;

do { 
	if(PeekMessage(&message,NULL,0,0,PM_REMOVE)){ 
		if ( WM_CLOSE == message.message && NULL == message.hwnd )
			PostMessage( windowHnd, WM_CLOSE, 0, 0 );
...
// ------------------------------------------------------------------------------
// Thread function for ScreenMonitor: C-style callback
// ------------------------------------------------------------------------------
unsigned int __stdcall ScreenMonitorThreadFunc( LPVOID lpParam ) 
{

	((ScreenMonitor *)lpParam)->create();
	((ScreenMonitor *)lpParam)->MessageLoop();
