///////////////////////////////////////////////////////////////
/// Chord Adaptation Dynamics with Spatial Visual Feedback - Ali Ghavampour, Amin Nazerzadeh May 2026
///////////////////////////////////////////////////////////////
...
void MyTrial::writeDat(ostream& out) {
	...
	out << subNum << "\t"
		...
		<< targetForce << "\t"					// target force for the single finger chord
		<< endForce << "\t"						// the force at the time of sampling execution (execSampleTime)
...
void MyTrial::updateGraphics(int what) {
	...
	if (gs.showTarget == 1) {
		x1 = ((gs.chord * FINGWIDTH) - 0.5 * (FINGWIDTH * N_FINGERS)) + FINGER_SPACING;
		x2 = (((gs.chord + 1) * FINGWIDTH) - 0.5 * (FINGWIDTH * N_FINGERS)) - FINGER_SPACING;
		xPos = (x1 + x2) * 0.5;
		xSize = x2 - x1;
		ySize = FLX_ZONE_WIDTH;
		yPos = gs.targetForce + VERT_SHIFT;
		gScreen.setColor(Screen::grey);
	...
	if (gs.showForces) {
		if (gs.isFrozenForces){
			diffForce[gs.chord] = (gs.frozenExtForces[gs.chord] - gs.frozenFlexForces[gs.chord]);
		}
		else{
...
void MyTrial::control() {
	...
	case WAIT_EXEC:
		...
		if ((beepInterval * 3 + execSampleTime - SAMPLING_DURATION/2) <= gTimer[1] && gTimer[1] < (beepInterval * 3 + execSampleTime + SAMPLING_DURATION/2)) { // NEEDS FIXING
			forceTemps[gs.chord] += VERT_SHIFT + forceGain * (gBox[0].getForce(gs.chord) - gBox[1].getForce(gs.chord));
			extForceTemps[gs.chord] += gBox[0].getForce(gs.chord);
			flexForceTemps[gs.chord] += gBox[1].getForce(gs.chord);
			samplingCounter++;
		}
		...
		if (gTimer[1] >= (beepInterval * 3 + execSampleTime + SAMPLING_DURATION / 2)) { // NEEDS FIXING
			...
			FinalFlexForces[gs.chord] = flexForceTemps[gs.chord] / samplingCounter;

			endForce = fingerForceTmp; // force at the time of giving feedback, saved in .dat file

			gs.isFrozenForces = 1;
			gs.frozenExtForces[gs.chord] = FinalExtForces[gs.chord];
			gs.frozenFlexForces[gs.chord] = FinalFlexForces[gs.chord];
