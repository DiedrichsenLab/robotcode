...
#include "ManipulandumRed3D_incom3.h"
...
//	Lgain = 0.000010;
	...
	//OmegaReal=Vector7D(0.020140, 0.049171, 1.401095, 0.313, 0.150, 0.313, 0.150); //omega from manual tuning
	//OmegaReal=Vector7D(0.0083,    0.0624,    1.2470,    0.2119,    0.4030,   0.3872,    0.1110); //omega from learning session2;
...
void Manipulandum::update(double dt){
	...
	rawPosition=forwardKinePos(theta); // calculate position of the wrist (3d) or endpoint (2d)
...
//	tDisp.setText(manipbuffer,18,col);
	...
	//fixed omega
	sprintf(manipbuffer,"W1:%3.3f, W2:%3.3f, W3:%3.3f, W4:%3.3f",OmegaReal[0],OmegaReal[1],OmegaReal[2],OmegaReal[3]);
	tDisp.setText(manipbuffer,16,col);

	sprintf(manipbuffer,"W5:%3.3f, W6:%3.3f, W7:%3.3f",OmegaReal[4],OmegaReal[5],OmegaReal[6]);
	tDisp.setText(manipbuffer,17,col);
...
void Manipulandum::selectWristTorqueControlUnsafe()		///> Sets wrist motor control to torque mode
{
	//if (wristFBOn==1) safeVoltGain=0; //DON'T de-ramp output so may jump!
	wristControlMode=WRISTTORQUE;
	//MAYBE RESET TARGET POSITION HERE!
...
{
//	if (gain>0.7) gain=0.7; //Temp safe hack for stability! Retune soon!
//	if (gain<0) gain=0;
...
void Manipulandum::updateWristAngleControl(double dt) ///< updates wrist angle PID controller 
{
		...
		//filter.position(positionFilt); // retrieve Kalman filter estimated position 
		wristAngleFilter.update(Vector2D(wristAError,0),dt); //update wrist error filter
		wristFilter.update(Vector2D(theta[2],0),dt); //update wrist angle filter
		...
		wristFilter.position(wristAVel);
		...
		fit_vel=wristFFDeadZone*sign(wristAVel[0])+((wristFFSlopeZone)*wristAVel[0]);

		//angular velocity ramp to stop dithering around zero angular velocity 
		//double AvClip=0.15;
		//double Avmag=wristAVel[0]; //get normalised velocity
		//if (Avmag>AvClip) Avmag=AvClip; //clamp at clip limit

		//double AvClipGain=0.5*cos(((Avmag/AvClip)+1)*PI)+0.5; 

		//fit_vel=fit_vel*AvClipGain;
...
void Manipulandum::updateWristTorqueControl(double dt) ///< updates wrist angle PID controller
{
		...
		wristTorqueFilter.update(Vector2D(wristTError,0),dt); //update wrist torque error filter
		wristFilter.update(Vector2D(theta[2],0),dt); //update wrist angle filter
		...
		wristFilter.velocity(wristAVel);
